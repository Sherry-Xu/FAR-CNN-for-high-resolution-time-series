...
for ivar=1:nvar
    for icurve=1:ncurve
        ...
        hessmat = hesscal(betai, Dyhat, wtroot, lambda, Kmat, inactive);
        ...
        for iter = 1:iterlim
            ...
            if Fstri.f > Foldstr.f
                ...
                hessmat = ...
                    hesscal(betai, Dyhat, wtroot, lambda, Kmat, inactive);
...
if ninactive > 0, Fstri.grad(inactive) = 0; end
Fstri.norm = sqrt(sum(Fstri.grad.^2));   %  gradient norm

%  ----------------------------------------------------------------

function hessmat = hesscal(betai, Dyhat, wtroot, lambda, Kmat, inactive)
nbet = length(betai);
[n, nbasis] = size(Dyhat);
