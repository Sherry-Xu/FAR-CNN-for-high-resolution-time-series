function [betastderrcell, bvar, c2bMap] ...
               = fRegress_stderr(fRegressStr, y2cMap, SigmaE)
...
%  Last modified 2 October 2009 by Jim Ramsay

%  get number of independent variables 
    
yfdPar   = fRegressStr.yfdPar;
xfdcell  = fRegressStr.xfdcell;
betacell = fRegressStr.betacell;
Cmatinv  = fRegressStr.Cmatinv;
...
if isa_fdPar(yfdPar) || isa_fd(yfdPar)
    ...
    if isa_fd(yfdPar), yfdPar = fdPar(yfdPar);  end
    ...
    for j=1:p
        ...
        if strcmp(class(xfdj), 'fd')
            ...
            bstderrfdj = fd(bstderrj', create_constant_basis([0,1]));
        end
        betastderrcell{j} = bstderrfdj;
    end
end
