...
liptime  = (0:50)';  %  sampling points for each curve

%  ----------  set up the b-spline basis object  ------------
%       use order 6 splines so we can look at acceleration

lipbasis = create_bspline_basis([0,50], 31, 6);
...
lambda   = 1e-8;       %  use light smoothing
...
subplot(1,1,1)
for i = 1:nobs
  plot(liptime, D2lipmat(:,i), 'o', [0,50], [0,0], ':')
...
nbasis = 6;
norder = 4;
breaks = [0,lipmeanmarks,50];
warpbasis = create_bspline_basis([0,50], nbasis, norder, breaks);
...
line([lipmeanmarks(1),lipmeanmarks(1)],[0,50])  % first knot
line([lipmeanmarks(2),lipmeanmarks(2)],[0,50])  % second knot
...
plot(liptime, lipdefmat, '-', [0,50], [0,0], ':')
...
nwbasis = 21;
wbasis = create_bspline_basis([0,50], nwbasis);
